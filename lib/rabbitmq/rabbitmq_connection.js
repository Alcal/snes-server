        ...
        try {
            ...
            for (const [consumerName, consumerConfig] of Object.entries(RMQ_CONSUMERS_CONFIG)) {
                this.consumers[consumerName] = this.connection.createConsumer({
                    ...
                    async (msg) => {
                        const ackStatus = await rabbitmqConsumers[consumerConfig.ROUTING_KEY](msg, this.consumerCallbacks[consumerName] ?? DEFAULT_CONSUMER_CALLBACK);
                        return ConsumerStatus[ackStatus ?? ConsumerStatus.DROP];
                    }
                );
